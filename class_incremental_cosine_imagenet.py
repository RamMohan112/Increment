...
parser.add_argument('--nb_cl_fg', default=5, type=int, \
    help='the number of classes in first group')
parser.add_argument('--nb_cl', default=2, type=int, \
...
lr_strat               = [20, 40]       # Epochs where learning rate gets decreased
...
# Initialization
dictionary_size     = 500
top1_acc_list_cumul = np.zeros((11,3,args.nb_runs))
top1_acc_list_ori   = np.zeros((11,3,args.nb_runs))
...
for iteration_total in range(args.nb_runs):
    ...
    Y_protoset_cumuls = []
    Y_train_cumuls    = []
    alpha_dr_herding  = np.zeros((11, dictionary_size),np.float32)
    ...
    for iteration in range(5, 12, 2):
        ...
        # Prepare the training data for the current batch of classes
        actual_cl        = order[range(start, end)]
        indices_train_10 = np.array([i in order[range(start, end)] for i in Y_train_total])
        indices_test_10  = np.array([i in order[range(start, end)] for i in Y_valid_total])
        ...
        if iteration > start_iter and args.imprint_weights:
            ...
            for cls_idx in range(iteration-2, iteration):
                ...
                novel_embedding[cls_idx-(iteration-2)] = F.normalize(cls_embedding, p=2, dim=0) * average_old_embedding_norm
            tg_model.to(device)
        ...
        for iter_dico in range(start, end):
            ...
            iter_herding     = 0
            iter_herding_eff = 0
            while not(np.sum(alpha_dr_herding[iter_dico,:]!=0)==min(nb_protos_cl,500)) and iter_herding_eff<1000:
                tmp_t   = np.dot(w_t,D)
        ...
        for iteration2 in range(end):
            
            current_cl = order[range(0,end)]
            ...
            # iCaRL
            alph = alpha_dr_herding[iteration2,:]
            ...
            X_protoset_cumuls.append(prototypes[iteration2][np.where(alph==1)[0]])
            Y_protoset_cumuls.append(order[iteration2]*np.ones(len(np.where(alph==1)[0])))
            alph = alph/np.sum(alph)
            class_means[:,current_cl[iteration2],0] = (np.dot(D,alph)+np.dot(D2,alph))/2
            class_means[:,current_cl[iteration2],0] /= np.linalg.norm(class_means[:,current_cl[iteration2],0])

            # Normal NCM
            # alph = np.ones(dictionary_size)/dictionary_size
            alph = np.ones(num_samples)/num_samples
            class_means[:,current_cl[iteration2],1] = (np.dot(D,alph)+np.dot(D2,alph))/2
            class_means[:,current_cl[iteration2],1] /= np.linalg.norm(class_means[:,current_cl[iteration2],1])

        torch.save(class_means, \
            './checkpoint/{}_run_{}_iteration_{}_class_means.pth'.format(args.ckp_prefix,iteration_total, iteration))

        current_means = class_means[:, order[range(0, end)]]
        ##############################################################
