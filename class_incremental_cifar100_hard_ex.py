...
from compute_hard_exemplars import compute_exemplars
######### Modifiable Settings ##########
...
for iteration_total in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        alpha_dr_herding_2 = np.zeros(alpha_dr_herding.shape)
        for iter_dico in range(last_iter*args.nb_cl, (iteration+1)*args.nb_cl):
            ...
            evalset.test_labels = np.ones(evalset.test_data.shape[0])*order_list.index(iter_dico) ##IMP Check 
            print(f"Label = {order_list.index(iter_dico)}")
            ...
            print(f"Mapped Prototypes = ", {mapped_prototypes.shape})
            ...
            print(f"Hard Exemplar outputs")
            alpha_dr_herding_2 = compute_exemplars(tg_model, evalloader, index1=index1, index2=index2, num_samples, alpha_dr_herding, nb_protos_cl) # Call for hard ex 
            ...
            while not(np.sum(alpha_dr_herding[index1,:,index2]!=0)==min(nb_protos_cl,500)) and iter_herding_eff<1000:
                print(f" w_t = {w_t.shape} D = {D.shape}")
                tmp_t   = np.dot(w_t,D)
                ind_max = np.argmax(tmp_t)
                print(f"tmp_t = {tmp_t.shape} , /t ind_max = {ind_max} ")
                iter_herding_eff += 1
